...
//const { customFfmpeg } = require('../lib/functions')
let handler  = async (m, { client, args }) => {
  let img = false
  try {
    let q = m.quoted ? { message: { [m.quoted.mtype]: m.quoted }} : m
    if (/^(image|video)/.test((m.quoted ? m.quoted : m.msg).mimetype || '')) {
    if (/video/.test((m.quoted ? m.quoted : m.msg).mimetype || '') && q.message.videoMessage.seconds > 15) throw 'Durasi video terlalu panjang!, max 15 detik'
    if (/video/.test((m.quoted ? m.quoted : m.msg).mimetype || '')) m.reply(`*_Tunggu sebentar sticker masih di prosess_*`)
    img = await client.downloadM(q)
  
    }else throw 'Reply/beri caption video/image'
  } finally {
    if (/video/.test((m.quoted ? m.quoted : m.msg).mimetype || '')) handler.limit = true
    else handler.limit = false
    if (img) client.sendFile(m.chat, img, '', '', m, {asSticker: true})
  }
}
handler.help = ['sticker', 'stickergif'].map(v => v + ' (caption|reply media)')
handler.tags = ['convert']
handler.command = /^stic?ker(gif|)$/i
handler.owner = false
handler.mods = false

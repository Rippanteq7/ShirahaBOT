let handler = async (m, { client, text }) => {
  client.hartatahta = client.hartatahta ? client.hartatahta : {}
  if (m.chat in client.hartatahta) throw 'Masih ada yang sedang membuat\nTeks Harta Tahta\ndi chat ini... tunggu sampai selesai'
  else client.hartatahta[m.chat] = true
  if (!text) throw 'Teks nya?'
  m.reply('_Sedang membuat..._\n*Mohon tunggu sekitar 1 menit*')
  try {
    let img = await ht(text)
    client.sendFile(m.chat, img, 'Harta Tahta.png', '*Using FFMPEG*\nBy Nurutomo', m)
  } finally {
    delete client.hartatahta[m.chat]
  }
}
